...
const logicalOperatorRegExp = ['\\|', '&'];
const relationalOperatorRegExp = ['==', '!=', '<=', '>=', '<', '>'];
...
String _recursion(String content) {
  for (var bracketMatch in RegExp(r'\(|\)').allMatches(content)) {
    ...
    if (bracket == '(') {
      list.add(Bracket2Index(bracket: bracket, index: bracketMatch.start));
    } else if (bracket == ')') {
      if (list.isEmpty) throw '缺少前括号！';
      final bracketInternal = content.substring(list.last.index + 1, bracketMatch.start).trim();
      ...
      if (bracketInternal.contains(RegExp(allOperatorRegExp.map((e) => '($e)').join('|'))) ||
        ...
        afterReplace = content.replaceAll(content.substring(list.last.index, bracketMatch.end), boolResult);
      } else {
        // TODO: 计算
        final calResult = 111111.toString();
        afterReplace = content.replaceAll(content.substring(list.last.index, bracketMatch.end), calResult);
      }
