...
class Ab<V> {
  ...
  /// 将当前对象标记为将要被重建。
  Ab<V> mark<C extends AbController>(C controller) {
...
extension ListBrokenExt on List {
  void removeAtForAb<C extends AbController>(C controller, int index) {
    final element = this[index];
    if (element is Ab) element.broken(controller);
    removeAt(index);
  }

  bool removeForAb<C extends AbController>(C controller, Object? element) {
    if (element is Ab) element.broken(controller);
    return remove(element);
  }

  void removeLastForAb<C extends AbController>(C controller) {
    if (last is Ab) last.broken(controller);
    removeLast();
  }

  void removeRangeForAb<C extends AbController>(C controller, int start, int end) {
  ...
  void clearForAb<C extends AbController>(C controller) {
    forEach((element) {
      if (element is Ab) element.broken(controller);
    });
    clear();
...
extension SetBrokenExt on Set {
  bool removeForAb<C extends AbController>(C controller, Object? element) {
    if (element is Ab) element.broken(controller);
    return remove(element);
  }

  void removeAllForAb<C extends AbController>(C controller, Iterable<Object?> elements) {
  ...
  void clearForAb<C extends AbController>(C controller) {
    removeAllForAb(controller, this);
  }
}
...
extension MapBrokenExt on Map {
  bool removeForAb<C extends AbController>(C controller, Object? key) {
